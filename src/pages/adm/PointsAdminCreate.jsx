...
function formDataToJsonMapper(id, customerCpfOrCnpj, points_customer, obs) {
  let formatedData = {
    id: id,
...
async function formHandle(e) {
  ...
  const jsonData = formDataToJsonMapper(idC, customerCpfOrCnpj, points_customer, obs);



  try {
    await axios.post(BACKEND_URL + '/admin/order/create', jsonData, axiosConfig)
    window.location = '/admin/points'
    alert('Pedido de pontuação criado com sucesso')
  }
  catch (error) {
    alert(error.response.data.message)
  }
...
export default function PointsAdminCreate() {
  if (!Cookies.get('AuthorizationAdmin')) {
          ...
          <div className="py-12 bg-primaria  transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0" id="modal">
            <div role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
              <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                ...
                <h1 className="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Criar Pontuação</h1>
