...
async function activeHandle(active, id) {
  ...
  try {
    await axios.patch(BACKEND_URL + '/admin/customers/state', jsonData, axiosConfig)
...
async function deleteHandle(id) {
  ...
  const sure = confirm("Você está excluindo um usuário(não recomendado!)\ntem certeza?");
  if (sure) {
    try {
      await axios.delete(BACKEND_URL + '/admin/customers/' + idD, axiosConfig)
      window.location = '/admin/customers'
    ...
    <>
      ...
      <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-lg text-center mb-5">
          <h1 className="text-2xl font-bold sm:text-3xl text-primaria">Profissionais</h1>
        </div>
        ...
        <article className="flex items-center gap-4 rounded-lg border border-terciaria bg-primaria p-6 mb-5">
          ...
          <div className="ml-auto mr-auto">
            ...
            <p className="text-sm text-gray-300">Total de Profissionais</p>
          </div>
        </article>
          ...
          <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
            <thead className="text-left">
              <tr>
                ...
                <th className="whitespace-nowrap py-2 font-medium text-gray-900">Cpf/Cnpj</th>
              ...
              {dataUser[0].map((item) => {
                return (
                  ...
                  <tr id={item.id}>
                    ...
                    <td className="whitespace-nowrap pl-1 py-2 text-gray-700">{item.cpfOrCnpj}</td>
                    <td className="whitespace-nowrap  py-2">
                      <a
                      onClick={() => {
                        window.location = '/admin/customers/edit?id='+item.id
                      }} 
