...
async function activeHandle(active, id) {
  ...
  try {
    await axios.patch(BACKEND_URL + '/admin/stores/state', jsonData, axiosConfig)
...
async function deleteHandle(id) {
  ...
  const sure = confirm("Você está excluindo uma loja(não recomendado!)\ntem certeza?");
  if (sure) {
    try {
      await axios.delete(BACKEND_URL + '/admin/stores/' + idD, axiosConfig)
      window.location = '/admin/stores'
      alert('Loja Deletada!')
                ...
                return (
                  ...
                  <tr id={item.id}>
                    ...
                    <td className="whitespace-nowrap  py-2">
                      <a
                      onClick={() => {
                        window.location = '/admin/stores/edit?id='+item.id
                      }} 
